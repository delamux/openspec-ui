...
- [x] 1.4 RED→GREEN `add`: append `- [ ] <id> <text>` to the last group; derive id `<lastGroupNumber>.<next>`; ignore empty text
- [x] 1.5 Locator guard: match by id + expected current text; throw a `stale` `DomainError` on drift; tasks with empty id are not editable
...
- [x] 2.1 Extend `ChangeRepository` port with `toggleTask` / `editTaskText` / `deleteTask` / `addTask` (projectPath, changeName, payload); update `InMemoryChangeRepository` to apply `applyTaskEdit` to its seeded text, with tests
- [x] 2.2 RED→GREEN `FileSystemChangeRepository` write methods: resolve change dir → read `tasks.md` → `applyTaskEdit` → write back; integration tests use a real temp dir and assert round-trip on disk
- [x] 2.3 Surface missing `tasks.md` and stale-match as `DomainError` (no stack traces leaked)
