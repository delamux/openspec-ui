...
## Purpose
How the app presents itself on small screens and on touch devices: how the app bar
stacks at phone widths, where hover-revealed controls go when nothing can hover, how
large a hit area a coarse pointer gets, and what must never widen the page. Layout
rules key on viewport width (phone at 640px, tablet at 768px); input rules key on
pointer capability, never on width.
## Requirements
